...
public class SubcribersOnHistory {
    ...
    //getListSubscribers
    private void getSubscribersOfChannel(SubscriberOnCampaignV2Listener s, String key, String id)
    {
        compositeDisposable = new CompositeDisposable();
        subChannelItem = new SubChannelItem();

        subscriberOnCampaignV2Listener = s;
    ...
    public void getListSubscripbers(SubscriberOnCampaignV2Listener s, String key, List<String> listIdChannel)
    {
        listSubChannelItem = new ArrayList<>();
        sizeListSubscribers = listIdChannel.size();
        for(int i = 0; i<listIdChannel.size();i++)
        {
            getSubscribersOfChannel(s,key,listIdChannel.get(i));
        }
    }
